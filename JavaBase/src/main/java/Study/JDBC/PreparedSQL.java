...
public class PreparedSQL<T> extends TypeUtils<T> {
	...
	private void preparedSQL(T entity, Method method, int i) throws SQLException, Exception {
	...
	public void preparedInsertSQL(T entity, List<Method> list, String AutoIncrementColumn)
		...
		while (iter.hasNext()) {
			...
			preparedSQL(entity, method, ++i);
		}

	}

	public void preparedUpdateSQL(T entity, List<Method> list, List<String> where) throws SQLException, Exception {
		int i = 0;
		int whereIndex = 0;
		if (where != null) {
		...
		while (iter.hasNext()) {
			...
			if (where != null && where.contains(columnName)) {
				preparedSQL(entity, method, ++whereIndex);
			} else {
				preparedSQL(entity, method, ++i);
			}
	...
	public void preparedDeleteSQL(T entity, List<Method> list, List<String> where) throws SQLException, Exception {
		preparedWhereSQL(entity, list, where);
	}

	public void preparedSelectSQL(T entity, List<Method> list, List<String> where) throws SQLException, Exception {
		preparedWhereSQL(entity, list, where);
	}

	public void preparedWhereSQL(T entity, List<Method> list, List<String> where) throws SQLException, Exception {
		int i = 0;
