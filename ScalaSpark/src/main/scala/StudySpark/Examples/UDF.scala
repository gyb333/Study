...
object UDF {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data = spark.range(1, 101)
    val udaf = new UDAFunction()
    
    //    data.createTempView("v_range")
    ...
    val result = data.agg(udaf($"id").as("udaf"))
    result.show()
    spark.stop()

  }

  class UDAFunction extends UserDefinedAggregateFunction {
    //输入数据的类型
